...
caminho = 'C:\Users\victo\OneDrive\Área de Trabalho\UFV\Nias\Numero_Deteccoes_consecutiva\Sinais\';
...
load('parametros.mat', 'parametros')
...
for cont_vol = 1:size(Vvoluntario,1) %fazer por voluntário 
     ...
     [dr,time] = protocolo_deteccao_qsample(x, parametros);
     %[dr,time] = protocolo_deteccao_qsample_2013(x, parametros2013, limiares2013);
     %[dr,time] = protocolo_deteccao_qsample_2015(x, parametros_2015_otim, limiares_2015_otim);
     %[dr,time] = protocolo_deteccao_qsample(x, parametros2);
...
%450Hz - 9, 720Hz, 9%
binsM = [82    84    86    88    90    92    94    96];
%freq. 81Hzoll,83,85,87,89,91,93,95Hz
%TXD = mean(mean(Tdr2,3),1)';
TXD = mean(mean(Tdr(binsM,:,:),3),1)';

binsR = 70:104;
binsR(11:2:29) = [];
%binsR = [binsR 157:163 237:243];
%binsR([31 38]) = [];
...
linf = 0;
lsup = 0.0673;
%inter = find(FP>linf & FP<lsup);
inter = 1:length(FP);
% mostrar resultados 
%1 - Taxa de detecção 
figure 
plot(TXD(inter),'.k','MarkerSize',10)
hold on     
plot([0 size(TXD(inter),1)],[mean(TXD(inter)) mean(TXD(inter))], ':r','LineWidth',2)
ylabel('Taxa de Detecção (%)','fontsize',12)
xlabel('Indice de parâmetros','fontsize',12)

%2 - Falsos Positivo

figure 
plot(FP(inter),'.k','MarkerSize',10)
hold on 
plot([0 size(FP(inter),1)],[linf linf], ':r','LineWidth',2)
plot([0 size(FP(inter),1)],[lsup lsup], ':r','LineWidth',2)
plot([0 size(FP(inter),1)],[mean(FP(inter)) mean(FP(inter))], ':b','LineWidth',1)
...
timeM = Ttime(binsM,inter,:); 
timeM(timeM==-1) = Mmax;
timeM = mean(mean(timeM,3),1)'*1; %1segundo por janela
TXD = mean(mean(Tdr(binsM,inter,:),3),1)' *100;
FP = mean(mean(Tdr(binsR,inter,:),3),1)' *100;
%parametros_par = parametros_2015_otim(inter,:);
parametros_par = parametros(inter,:);

figure1 = figure; axes1 = axes('Parent',figure1); hold(axes1,'on');  
plot([0 1]*100, [240 ,240],'-.k','linewidth',1) 
plot([TXD(1) TXD(1)], [min(timeM) max(timeM)],'-.k','linewidth',1) 

index = find(parametros_par(:,1));
for ii = 1:size(parametros_par,1)
    plot(TXD(ii),timeM(ii),'.k','Markersize',6,'DisplayName',[num2str(parametros_par(ii,1)) '-' num2str(parametros_par(ii,2))])
end
...
ylabel('Tempo médio de exame (min)','fontsize',12);
fprintf('\n'); 
...
for ii = 1:size(idxs,1)
    ...
    for jj = I  
        ...
        fprintf(' - Buffer:%d, M_step:%d', parametros_par(jj,3),parametros_par(jj,2)); 
...
%        text(PD_T(jj)*99,T_exame_G(idxs(ii))*.985,['\{' num2str(parametros(jj,1)) ',' num2str(parametros(jj,2)) ',100\}' ]);
       text(TXD(jj),timeM(idxs(ii))*.975,['\{' num2str(parametros_par(jj,1)) ',' num2str(parametros_par(jj,2)) '\}' ],'FontSize',6);
    end    
end
fprintf('\n'); 
xlim([min(TXD(idxs))*.95,max(TXD(idxs))*1.05])
ylim([min(timeM(idxs))*.95 240*1.05])
