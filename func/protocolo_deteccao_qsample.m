function [dr,time] = protocolo_deteccao_qsample(x, parametros)
...
for M = 1:binsM %fazer para cada acrescimo de uma janela       
   ...
   for N = 1:size(x,2)
       Y = xfft([M M*2 M*3],1:N);%,'ComparisonMethod','abs'); % Colocar em ordem crescente em relação ao módulo
...
for ii = 1:size(parametros,1)
    ...
    %L = parametros(ii,39);
    % protocolo
    %stop = find(parametros(ii,:) == 0);
    %MM = parametros(ii,1:(stop-1));
    MM = Mmin:Mstep:Mmax;
